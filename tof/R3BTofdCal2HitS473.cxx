...
InitStatus R3BTofdCal2HitS473::Init()
{
    ...
    //determine factor for velocity correction
	if(fCalEnergy != fBeamEnergy){
		Double_t gamma0sq = pow((940.0 + fCalEnergy)/940.,2);
		Double_t gamma1sq = pow((940.0 + fBeamEnergy)/940.,2);

		Double_t B = 2.*511./0.035;

		Double_t Edep0 = gamma0sq/(gamma0sq-1.)* TMath::Log(B*(gamma0sq - 1.)) - 1.;
		Double_t Edep1 = gamma1sq/(gamma1sq-1.)* TMath::Log(B*(gamma1sq - 1.)) - 1.;
...
Double_t R3BTofdCal2HitS473::saturation(Double_t x)
{
    Double_t kor;
    Int_t voltage = 600;
    if (voltage == 600)
