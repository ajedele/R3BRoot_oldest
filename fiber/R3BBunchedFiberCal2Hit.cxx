...
R3BBunchedFiberCal2Hit::R3BBunchedFiberCal2Hit(const char* a_name,
    ...
    , fName(a_name)
    , fClockFreq(R3BTCalEngine::CTDC_8_12_FWD_250 == a_ctdc_variant ? 150 : 250)
...
InitStatus R3BBunchedFiberCal2Hit::Init()
{
    ...
    fh_ToT_MA_Fib = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 1000, 0., 400.);
    ...
    fh_Fib_ToF = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 44000, -1100., 1100.);
    ...
    fh_Test = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 50, -1024., 1024.);
    ...
    chistTitle = fName + " ToT of fibers";
    fh_ToT_Single_Fib = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 100, 0., 100.);
    ...
    for (Int_t i = 0; i < 4; i++)
    {
        ...
        fh_ToT_s_Fib[i] = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 100, 0., 100.);
    ...
    fh_dt_Fib = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 1000, -5000., 5000.);
    ...
    fh_time_Fib = new TH2F(chistName.Data(), chistTitle.Data(), 1024, 0, 1024, 4000, -2000., 2000.);
...
void R3BBunchedFiberCal2Hit::Exec(Option_t* option)
{
    ...
    for (auto it_mapmt = mapmt_array.begin(); mapmt_array.end() != it_mapmt; ++it_mapmt) // over fiber_number 0...nmax-1
    {
             ...
             ++it_mapmt_tot) // over ihit(fiber)
        {
            ...
            //auto fiber_id = mapmt_tot.lead->GetChannel();
            					auto fiber_id = (mapmt_tot.lead->GetChannel() - 1) * fChPerSub[1]
                                                            + ((spmt_tot.lead->GetChannel() - 1) % fChPerSub[1]) +
             1;
	    ...
	    //Double_t gainS = gain_temp[fiber_id - 1]; // 10.;
	    Double_t gainS = 10.;
            Double_t offset1 = 0.;
            Double_t offset2 = 0.;
            ...
            if (fName == "Fi10" || fName == "Fi11" || fName == "Fi12" || fName == "Fi13")
            {
                Float_t fiber_thickness = 0.050000; // s remove *2 when taking SPMT into analysis
                Int_t fiber_nbr = 1024;              // s remove /2 when taking SPMT into analysis
            ...
            if (fName == "Fi1a" || fName == "Fi1b" || fName == "Fi2a" || fName == "Fi2b" || fName == "Fi3a" ||
                fName == "Fi3b")
            {
                Float_t fiber_thickness = 0.021000; // s remove *2 when taking SPMT into analysis
                Int_t fiber_nbr = 512;               // s remove /2 when taking SPMT into analysis
                Float_t dead_layer = 0.9;
                Float_t air_layer = 0.01; // relative to fiber_thickness
                Float_t detector_width = fiber_nbr * fiber_thickness * (1 + air_layer);
                if (fDirection == VERTICAL)
            ...
            if (!fIsCalibrator)
                new ((*fHitItems)[fNofHitItems++])
                                             R3BBunchedFiberHitData(0, x, y, eloss, t, fiber_id, t_mapmt, t_spmt,
                    tot_mapmt, tot_spmt);
