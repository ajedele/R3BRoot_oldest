...
void R3BPspxPrecal2Cal::Exec(Option_t* option)
{
    ...
    for (Int_t d = 0; d < fPrecalItems.size(); d++)
    {
        ...
        for (Int_t i = 0; i < nPrecal; i++)
        {
            ...
            Float_t time1 = (precalData->GetTime1() - precalData->GetTrigger());
            Float_t time2 = (precalData->GetTime2() - precalData->GetTrigger());
            Float_t timediff = (time1 - time2);
            Float_t time = (time1 + time2)/2.;
            if (timediff > 1000) {time=5000;}
            ...
            new ((*fCalItems[d])[fCalItems[d]->GetEntriesFast()]) R3BPspxCalData(strip, energy, time, pos);
        }
    }
}
